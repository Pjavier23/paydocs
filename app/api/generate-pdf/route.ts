...
// eval('require') is intentional: prevents Next.js bundler from inlining its
// own vendored React 19 canary. At runtime this resolves to native Node.js
// require, loading react and react-pdf from the same node_modules instance
// and eliminating the cross-version React element mismatch (error #31).
// eslint-disable-next-line no-eval
const _req = eval('require') as NodeRequire
const React = _req('react') as typeof import('react')
const { renderToBuffer, Document, Page, Text, View, StyleSheet } =
  _req('@react-pdf/renderer') as typeof import('@react-pdf/renderer')
...
function PaystubPDF({ d }: { d: PaystubData }) {
  ...
  const copy = (label: string) => React.createElement(Page, { size: 'LETTER', style: styles.page },
...
function Invoice_PDF({ d }: { d: InvoiceData }) {
  ...
  return React.createElement(Document, null,
    React.createElement(Page, { size: 'LETTER', style: styles.page },
      ...
      React.createElement(View, { style: { flexDirection: 'row', backgroundColor: '#0f0f1a', padding: '6 8' } },
      ...
      ...d.items.map((item, idx) =>
        React.createElement(View, { key: item.id, style: { flexDirection: 'row', padding: '5 8', backgroundColor: idx % 2 === 0 ? '#f9f9f7' : 'white', borderBottomWidth: 0.5, borderBottomColor: '#e5e7eb' } },
...
export async function GET(req: NextRequest) {
  ...
  try {
    let element
    if (doc.type === 'paystub') element = React.createElement(PaystubPDF, { d: doc.data_json as PaystubData })
    else if (doc.type === '1099-nec') element = React.createElement(NEC_PDF, { d: doc.data_json as Form1099NECData })
    else if (doc.type === 'invoice') element = React.createElement(Invoice_PDF, { d: doc.data_json as InvoiceData })
    else element = React.createElement(MISC_PDF, { d: doc.data_json as Form1099MISCData })
