...
// without any React instance conflicts.
...
function kvRow(label: string, value: string): Row[] {
  return [
    {
      columns: [
        { text: label, color: GRAY, fontSize: 9 },
...
function hrBold(label: string, value: string): Row[] {
  return [
    {
      canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.5, lineColor: BORDER_LIGHT }],
      marginBottom: 3,
      marginTop: 2,
    },
    {
      columns: [
        ...
        { text: value, alignment: 'right', bold: true, fontSize: 9 },
      ],
      marginBottom: 3,
    },
  ]
...
function paystubPage(d: PaystubData, copyLabel: string): Row[] {
  const totalDed =
    d.federalTax + d.stateTax + d.socialSecurity + d.medicare + d.healthInsurance + d.otherDeduction
  ...
  const deductionRows: Row[] = [
    ...
    ...(d.otherDeduction > 0
      ? kvRow(d.otherDeductionLabel || 'Other', fmt(d.otherDeduction))
      : []),
  ...
  const earningRows: Row[] = [
    ...kvRow(
      d.payType === 'hourly'
        ? `Regular (${d.hoursWorked} hrs @ ${fmt(d.hourlyRate)})`
        : 'Salary',
      fmt(d.grossPay),
    ),
    ...(d.overtimeHours > 0
  ...
  return [
      ...
      columns: [
        {
          stack: [
            ...
            {
              text: `Pay Date: ${d.payDate}  |  Period: ${d.payPeriodStart} – ${d.payPeriodEnd}  |  Check #${d.checkNumber}`,
              fontSize: 8,
              color: GRAY,
              marginBottom: 10,
            },
    ...
    {
      canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 2, lineColor: DARK }],
      marginBottom: 8,
    },
    {
      columns: [
        ...
        },
        {
          stack: [
            ...
            { text: fmt(netPay), fontSize: 20, bold: true, alignment: 'right' },
          ],
          alignment: 'right',
        },
      ],
...
function buildPaystub(d: PaystubData): Record<string, unknown> {
  return {
...
function buildNEC(d: Form1099NECData): Record<string, unknown> {
  return {
    defaultStyle: { font: 'Helvetica', fontSize: 9, color: DARK },
    pageSize: 'LETTER',
    pageMargins: [36, 36, 36, 36],
    content: [
      ...
      },
      {
        columns: [
          {
            stack: [
              sectionLabel('BOX 1 — NONEMPLOYEE COMPENSATION'),
              { text: fmt(d.nonemployeeComp), fontSize: 18, bold: true },
            ],
            width: '*',
          },
          {
            stack: [
              sectionLabel('BOX 4 — FEDERAL TAX WITHHELD'),
              { text: fmt(d.federalTaxWithheld), fontSize: 14, bold: true },
            ],
            width: 'auto',
          },
          {
            stack: [
              sectionLabel(`BOX 6 — ${d.stateCode} TAX WITHHELD`),
              { text: fmt(d.stateTaxWithheld), fontSize: 14, bold: true },
            ],
            width: 'auto',
          },
        ],
...
function buildMISC(d: Form1099MISCData): Record<string, unknown> {
  ...
  const boxContent = boxes.map(([label, val]) => ({
    stack: [
      sectionLabel(label),
      { text: fmt(val), fontSize: 13, bold: true },
    ],
    marginBottom: 8,
  }))
...
function buildInvoice(d: InvoiceData): Record<string, unknown> {
  ...
  const totalsRows: Row[][] = [
    [
      ...
      { text: '', border: [false, false, false, false] },
      { text: 'Subtotal', color: GRAY, fontSize: 9, alignment: 'right', border: [false, false, false, false] },
      { text: fmt(subtotal), fontSize: 9, alignment: 'right', border: [false, false, false, false] },
    ] as Row[],
  ]
  if (discountAmt > 0) {
    totalsRows.push([
      { text: '', border: [false, false, false, false] },
      { text: '', border: [false, false, false, false] },
      { text: `Discount (${d.discount}%)`, color: GRAY, fontSize: 9, alignment: 'right', border: [false, false, false, false] },
      { text: `– ${fmt(discountAmt)}`, color: '#ef4444', fontSize: 9, alignment: 'right', border: [false, false, false, false] },
    ] as Row[])
  }
  if (taxAmt > 0) {
    totalsRows.push([
      { text: '', border: [false, false, false, false] },
      { text: '', border: [false, false, false, false] },
      { text: `Tax (${d.taxRate}%)`, color: GRAY, fontSize: 9, alignment: 'right', border: [false, false, false, false] },
      { text: fmt(taxAmt), fontSize: 9, alignment: 'right', border: [false, false, false, false] },
    ] as Row[])
  }
  totalsRows.push([
    { text: '', border: [false, true, false, false] },
    { text: '', border: [false, true, false, false] },
    { text: `TOTAL (${d.currency})`, bold: true, fontSize: 11, alignment: 'right', border: [false, true, false, false] },
    { text: fmt(total), bold: true, fontSize: 11, alignment: 'right', border: [false, true, false, false] },
  ] as Row[])
    ...
    content: [
      {
        columns: [
          {
            stack: [
              { text: 'INVOICE', fontSize: 22, bold: true },
              { text: `#${d.invoiceNumber}`, color: GRAY, fontSize: 9, marginTop: 2 },
            ],
          },
          {
            stack: [
              { text: `Date: ${d.invoiceDate}`, color: GRAY, fontSize: 9, alignment: 'right' },
              { text: `Due: ${d.dueDate}`, color: GRAY, fontSize: 9, alignment: 'right', marginTop: 2 },
              { text: `Terms: ${d.paymentTerms}`, color: GRAY, fontSize: 9, alignment: 'right', marginTop: 2 },
            ],
          },
        ],
        marginBottom: 20,
      ...
      ...(d.notes
        ? [
            sectionLabel('NOTES'),
            { text: d.notes, fontSize: 9, color: '#374151', marginBottom: 4 },
          ]
        : []),
    ],
  }
